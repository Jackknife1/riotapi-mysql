...
connection = mysql.connector.connect(
    host = "localhost",
    user = "root",
    password = "LOL1v1hra", #Your server password
    database = "league_of_legends" #Your database name
)
...
#Querying matchid in database table
query = "SELECT COUNT(matchid) FROM matches WHERE matchid = %s"
mycursor.execute(query, (matchid,))


#Checks if query result is greater than one. If it is, inserts final list into database. If it's not, displays: Matchid already in database  
